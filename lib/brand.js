...
// Word boundaries matter for a rubber-products company: without them,
// generic queries like "rubber forming" and "rubber formulation" contain
// the substring "rubber form" and would be classified branded, inflating
// the branded share. \b after "form" rejects "forming"/"formulation".
const DEFAULT_BRAND = /\brubber[-\s]?form\b/i;

// RE2-safe variant for the GSC API's includingRegex filter (RE2 has no \b).
export const DEFAULT_BRAND_RE2 = '(^|[^a-z])rubber[- ]?form([^a-z]|$)';

export function getBrandRegexRe2() {
  return process.env.BRAND_QUERY_REGEX_RE2?.trim() || DEFAULT_BRAND_RE2;
}
